...
class Turma {
    ...
    getMediaAlturas() {
        if (this.listaDeAlunos.length === 0)
            return 0;
        const somaDasAlturas = this.listaDeAlunos.reduce((total, aluno) => total + aluno.altura, 0);
        return somaDasAlturas / this.listaDeAlunos.length;
    }
    getMediaPesos() {
        if (this.listaDeAlunos.length === 0)
            return 0;
        const somaDosPesos = this.listaDeAlunos.reduce((total, aluno) => total + aluno.peso, 0);
